...
{   
template<>
struct SecSuite<SEC_SUITE_ETHEREUM>
{
...
protected:
	...
	{	
		try{
			...
			Botan::ECDSA_PublicKey reproducedPublicKey(group, publicPoint);

			Botan::PK_Verifier verifier(reproducedPublicKey, "EMSA1(SHA-256)");
	...
	{	
		try{
			...
			Botan::ECDSA_PrivateKey keyPair(rng, ecGroup, bigInt);

			Botan::PK_Signer signer(keyPair, rng, "EMSA1(SHA-256)");
	...
	{	
		try {
			...
			Botan::ECDSA_PrivateKey keyPair(rng, ecGroup);
