...
enum SecSuiteId: uint8_t
{
	...
	SEC_SUITE_NO_SIGNING = 4,	// address is not sign-able at all
	SEC_SUITE_CONTRACT = SEC_SUITE_NO_SIGNING,	// it's a contract, converted from the contract's id
	SEC_SUITE_CUSTOM,							// some address procedurally generated by code
	SEC_SUITE_NO_SIGNING_MAX,
...
static_assert(uint8_t(SEC_SUITE_CRYPTOGRAPHY_MAX) <= uint8_t(SEC_SUITE_NO_SIGNING), "SEC_SUITE_CRYPTOGRAPHY section overlaps with SEC_SUITE_NO_SIGNING section");
static_assert(uint8_t(SEC_SUITE_NO_SIGNING_MAX) <= uint8_t(SEC_SUITE_DELEGATED), "SEC_SUITE_NO_SIGNING section overlaps with SEC_SUITE_DELEGATED section");
...
{	
	...
	void			SetAsCustom(const uint8_t* data);				// data is 32-byte long and copied to address
