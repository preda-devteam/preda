...
static void *Xz_SzAlloc(void *p, size_t size) { p = p; return MyAlloc(size); }
static void Xz_SzFree(void *p, void *address) { p = p; MyFree(address); }
static ISzAlloc g_Xz_Alloc = { Xz_SzAlloc, Xz_SzFree };
...
static size_t Xz_Write(void *pp, const void *data, size_t size)
{
    ...
    ICompressProgress *progress)
{
...

  {
    ...
    seqSizeOutStream.p.Write = Xz_Write;
...

    {
      ...
      RINOK(Xz_AddIndexRecord(xz, block.unpackSize, seqSizeOutStream.processed - padSize, &g_Xz_Alloc));
    }
  }
